...
export default function CalendarPage() {
  ...
  const events = data?.items?.map((item: any) => {
    ...
    if (item.type === 'exam') color = '#f43f5e';
    else if (item.type === 'task') color = '#10b981';
    else if (item.type === 'assignment') color = '#818cf8';
  ...
  return (
    <div className="container" style={{ padding: '2rem 1.5rem' }}>
      <div className="page-header">
        <h1 style={{ fontSize: '1.75rem', fontWeight: 800, color: '#e0e7ff', marginBottom: '0.25rem' }}>Master Calendar</h1>
        <p style={{ color: '#9ca3af', fontSize: '0.9rem' }}>All your assignments, exams, and study sessions in one view.</p>
      </div>
